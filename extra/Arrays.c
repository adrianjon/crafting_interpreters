...
void array_push(dynamic_array_t* array, void* element, size_t element_size) {
...
char* string_to_uppercase(const char* str) {
    ...
    char* upper_str = (char*)memory_allocate(len + 1);
...
char* string_to_lowercase(const char* str) {
    ...
    char* lower_str = (char*)memory_allocate(len + 1);
    for (size_t i = 0; i < len; i++) {
        char c = str[i];
